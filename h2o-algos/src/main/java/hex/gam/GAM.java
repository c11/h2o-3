...
public class GAM extends ModelBuilder<GAMModel, GAMModel.GAMParameters, GAMModel.GAMModelOutput> {
  ...
  public void cv_computeAndSetOptimalParameters(ModelBuilder[] cvModelBuilders) {
     ...
     * of single predictor smoothers to the front and multiple predictor smoothers to the back of the array.
     *
  ...
  public double[][][] generateKnotsFromKeys() { // todo: parallize this operation
    ...
    for (int outIndex = 0; outIndex < _parms._gam_columns.length; outIndex++) { // go through each gam_column group
      ...
      if (_parms._gam_columns[outIndex].length > 1) // thin plate regression
        gamIndex = tpInd++;
      else
        gamIndex = csInd++;
      ...
      if (tempKey != null && (tempKey.length() > 0)) {  // read knots location from Frame given by user      
        ...
        if (_parms._gam_columns[outIndex].length == 1) {
          ...
          int rowCount = 0;
          knots[gamIndex] = MemoryManager.malloc8d(_parms._gam_columns[outIndex].length, _parms._num_knots[outIndex]);
          ...
          for (int rowInd = 0; rowInd < randomRowVec.length; rowInd++) {
            ...
            for (int colInd = 0; colInd < _parms._gam_columns[outIndex].length; colInd++) {
              ...
              knots[gamIndex][colInd][rowCount] = predictVec.vec(_parms._gam_columns[outIndex][colInd]).at(randomRowVec[rowInd]);
            }
            if (!foundNAinRow)
              rowCount++;
          }
    ...
    for (int index = 0; index < _parms._gam_columns.length; index++) {
        if (_parms._bs_sorted[index] == 1 && _parms._num_knots_sorted[index] <= _parms._M[tpIndex]+1)
          error("num_knots", "num_knots for gam column start with  "+_parms._gam_columns_sorted[index][0]+
                  " did not specify enough num_knots.  It should be exceed "+(_parms._M[tpIndex++]+1)+".");
    }
  }
  ...
  private class GAMDriver extends Driver {
    ...
    void verifyGamTransformedFrame(Frame gamTransformed) {
      final int numGamCols = _gamColNamesCenter.length;
